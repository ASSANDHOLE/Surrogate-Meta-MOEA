...
class MrMetaActEncoder(nn.Module):
    def __init__(self, in_out_dim, hidden_size):
        super(MrMetaActEncoder, self).__init__()
        self.in_out_dim = in_out_dim
        self.hidden_size = hidden_size
        self.dense_proj = nn.Linear(in_out_dim, hidden_size)
        self.dense_mu = nn.Linear(hidden_size, in_out_dim)
        self.dense_sigma_w = nn.Linear(hidden_size, in_out_dim)

    def forward(self, x):
        h = F.relu(self.dense_proj(x))
        mu_w = self.dense_mu(h)
        sigma_w = F.softplus(self.dense_sigma_w(h))
        ws = mu_w + sigma_w * torch.randn_like(mu_w)
        return ws, mu_w, sigma_w

    @staticmethod
    def kl_qp_gaussian(mu_q, sigma_q, mu_p, sigma_p):
        sigma2_q = sigma_q + 1e-16
        sigma2_p = sigma_p + 1e-16
        tmp = torch.log(sigma2_p) - torch.log(sigma2_q) - 1.0 + \
            sigma2_q / sigma2_p + torch.square(mu_q - mu_p) / sigma2_p
        kl = 0.5 * torch.mean(tmp, dim=1)
        return torch.mean(kl)


class MrMamlAct(nn.Module):
    """
    Meta Learner
    """
    ...
    def __init__(self, args, config):
        ...
        self.net = Learner(config, imgc=1, imgsz=1)
        self.meta_optim = optim.Adam(self.net.parameters(), lr=self.meta_lr)
        self.problem_dim = args.problem_dim
        self.data_encoder = MrMetaActEncoder(self.problem_dim[0], 100)
        self.beta_enc = 5.0
    ...
    def forward(self, x_spt, y_spt, x_qry, y_qry):
        ...
        losses_enc = [0 for _ in range(self.update_step + 1)]

        for i in range(task_num):

            # 1. run the i-th task and compute loss for k=0
            enc_x_spt, _, _ = self.data_encoder(x_spt[i])
            logits = self.net(enc_x_spt, vars=None, bn_training=True)
            ...
            with torch.no_grad():
                # [setsz, nway]
                enc_x_qry, _, _ = self.data_encoder(x_qry[i])
                logits_q = self.net(enc_x_qry, self.net.parameters(), bn_training=True)
            ...
            with torch.no_grad():
                # [setsz, nway]
                logits_q = self.net(enc_x_qry, fast_weights, bn_training=True)
                logits_q = logits_q.reshape(y_qry[i].shape)
                loss_q = loss_func(logits_q, y_qry[i])
                losses_q[1] += loss_q
            ...
            for k in range(1, self.update_step):
                ...
                logits = self.net(enc_x_spt, fast_weights, bn_training=True)
                loss = loss_func(logits, y_spt[i])
                ...
                w_val, w_mu_val, w_sigma_val = self.data_encoder(x_qry[i])
                with torch.no_grad():
                    w_val_y = self.net(w_val, fast_weights, bn_training=True)
                    enc_loss = loss_func(w_val_y, y_qry[i])
                kl_ib = MrMetaActEncoder.kl_qp_gaussian(
                    w_mu_val, w_sigma_val,
                    torch.zeros_like(w_mu_val), torch.ones_like(w_sigma_val)
                )
                losses_enc[k + 1] += enc_loss + self.beta_enc * kl_ib

        # end of all tasks
        # sum over all losses on query set across all tasks
        loss_q = losses_q[-1] / task_num
        loss_enc = losses_enc[-1] / task_num
        ...
        enc_optim = torch.optim.Adam(self.data_encoder.parameters(), lr=0.001)
        enc_optim.zero_grad()
        loss_enc.backward()
        enc_optim.step()
    ...
    def _fine_tuning(self, x_spt, y_spt, x_qry, y_qry, net=None, return_single_lose=True):
        ...
        x_spt, _, _ = self.data_encoder(x_spt)
        if query_size != 0:
            x_qry, _, _ = self.data_encoder(x_qry)
        # 1. run the i-th task and compute loss for k=0
        logits = net(x_spt)
